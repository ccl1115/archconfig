...
for s = 1, screen.count() do
    ...
    tags[s] = awful.tag({ "android", "web", "term", "others" }, s, layouts[2])
end
-- }}}
...
client.connect_signal("manage", function (c, startup)
    ...
    -- if titlebars_enabled then
        ...
        -- Widgets that are aligned to the right
        local right_layout = wibox.layout.fixed.horizontal()
        right_layout:add(wibox.widget.textbox(c.class))
...
wp_timer:connect_signal("timeout", function()
    for s = 1, screen.count() do
        local path = wp_path .. wp_files[wp_index]
        gears.wallpaper.maximized(path, s, true)
        naughty.notify({ title = "Wallpaper set:",  text = "path"})
    end
